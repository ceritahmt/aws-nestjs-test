...
export class DepartmentService {
  ...
  async create(createDepartmentDto: CreateDepartmentDto): Promise<Department>  {
    ...
    if (existingDepartment) {
      throw new BadRequestException('Department with this code or name already exists');
    }
  ...
  async update(idOrCode: string, updateDepartmentDto: UpdateDepartmentDto):Promise<Department>{
    const existingDepartment = await this.departmentRepository.findOne({
      where: [
        { id: isNumberString(idOrCode)? parseInt(idOrCode) : 0  },
        { code: idOrCode }
    ...
    const res= await this.departmentRepository.update(existingDepartment.id,department);
  ...
  async remove(idOrCode: string):Promise<undefined>   {
    ...
    const res= await this.departmentRepository.delete(existingDepartment.id);
    return ;
  }
}
